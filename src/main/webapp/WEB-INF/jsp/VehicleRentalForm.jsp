...
$(document).ready(function(){
	...
	$('#bookedTo').blur(function () {
		...
		var month = sDate.split("/")[0];
		var day = sDate.split("/")[1];
		var reg = $('#registrationNumber').val();
		var year = sDate.split("/")[2];

		var month1 = eDate.split("/")[0];
		var day1 = eDate.split("/")[1];
...
<body>
	...
	<h3>
		...
		Username :
		<%=user%></h3>
	<form:form method="POST" name="vehicleRental"
		commandName="vehicleRental">
		...
		<table>
			...
			<tr>
				<td>Booked From</td>
				<td><form:input path="bookedFrom" id="bookedFrom"></form:input></td>
				<td><form:errors path="bookedFrom" cssClass="error" /></td>
			</tr>
			<tr>
				<td>Booked To</td>
				<td><form:input path="bookedTo"
						onblur="fnGetTotalRent(this.form)"></form:input></td>
